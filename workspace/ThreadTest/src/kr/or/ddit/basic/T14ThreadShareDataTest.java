...
//	     CPU는 속도가 빠르고 메모리는 CPU에 비해 느려서 효율적이지 않아 캐쉬메모리를 만들어 CPU와 
//	          캐쉬 메모리를 통해서 처리를 하는데 이 경우에 캐쉬메모리는 true로 바뀌지 않고 false만을 
//	          나타내고 있어 출력을 담당하는 CPU가 계속해서 false로 인식. 따라서 메인 메모리에 직접
//	          접근할 수 있게 해줘 true로 바뀌는 것을 확인하게 바꿔줌.                       
	 */
